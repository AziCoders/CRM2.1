...
from bot.utils.timezone import get_msk_now, get_msk_time, get_msk_date
...
class ReminderService:
    ...
    def should_send_reminder_now(self) -> bool:
        """
        Проверяет, нужно ли отправлять напоминание сейчас (по МСК)
        
        Returns:
            True если текущее время МСК совпадает с одним из времен напоминаний
        """
        now = get_msk_time()
    ...
    def should_send_payment_reminder_now(self) -> bool:
        """
        Проверяет, нужно ли отправлять напоминание о платежах сейчас (13:00 по МСК)
        
        Returns:
            True если текущее время МСК 13:00
        """
        now = get_msk_time()
    ...
    def should_send_unprocessed_students_reminder_now(self) -> bool:
        """
        Проверяет, нужно ли отправлять напоминание о необработанных учениках сейчас (10:00 по МСК)
        
        Returns:
            True если текущее время МСК 10:00
        """
        now = get_msk_time()
