...
Версия которая должна работать на Ubuntu 22.04
Попытка 2
"""
import json
...
def convert_city_en_to_ru(city_en: str) -> str:
    ...
    # Преобразуем английское название обратно в русское (как в back_to_students.py)
    city_name = None
...
async def handle_info_action(
    ...
    await callback.answer()  # Обязательный ответ в начале
...
async def handle_group_info(
    ...
    # Ищем группу по сокращенному ID (как в back_to_students.py)
    groups = group_service.get_city_groups(city_name)
    group = None
    group_id_full = None
...
async def handle_student_select(
    ...
    # Ищем ученика по сокращенному ID (как в back_to_students.py)
    student_data = None
    student_id_full = None
    
    # Нормализуем student_id_short - берем первые 16 символов
    student_id_short_normalized = student_id_short[:16] if len(student_id_short) > 16 else student_id_short
    ...
    for student in students:
        ...
        student_id_short_from_data = student_id_no_dashes[:16]  # Берем первые 16 символов
        # Используем точное сравнение первых 16 символов
        if student_id_short_from_data == student_id_short_normalized:
...
async def handle_back(
    ...
    try:
        ...
        if level == "main":
            ...
            if user_role in ["manager", "owner"]:
                ...
                if user_data:
                    ...
                    if user_city:
                        ...
                        await callback.answer("❌ Ошибка: не удалось определить ваш город", show_alert=True)
                else:
            ...
            # Неизвестный уровень
            await callback.answer("❌ Ошибка: неизвестный уровень навигации", show_alert=True)
